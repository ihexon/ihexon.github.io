---
title: "BinaryHub：预构建嵌入式二进制工具"
summary: "提供面向光猫等嵌入式设备的预构建 ELF 工具，例如 curl、wget、ftp 和 busybox，方便固件分析和测试环境搭建。"
description: "提供面向光猫等嵌入式设备的预构建 ELF 工具，例如 curl、wget、ftp 和 busybox，方便固件分析和测试环境搭建。"
date: 2023-01-27
draft: false
categories:
  - "embedded"
...
你会发现单个二进制文件体积有点大，因为我保留了 debug symbol，因为某些 ELF 文件 运行的不是很好，需要调试，保留 debug symbol 你也自己可以帮忙调试。
