---
title: "_dl_fixup 机制研究记录"
summary: "记录 glibc 动态链接器 _dl_fixup 的调试环境、关键局部变量、符号查找和重定位修正流程。"
description: "记录 glibc 动态链接器 _dl_fixup 的调试环境、关键局部变量、符号查找和重定位修正流程。"
date: 2023-10-20
draft: false
categories:
  - "binary-analysis"
...
$ make ; make install
```
> glibc 不支持 Clang 编译，或者说暂时不支持。因为 glibc 本身使用了大量GCC独占的特性。但开源社区也有[尝试用 Clang 构建 glibc](https://www.linaro.org/blog/building-glibc-with-llvm-the-how-and-why/)。
...
- readelf
- elixir.bootlin.com
    可以到 [elixir.bootlin.com](https://elixir.bootlin.com/glibc/glibc-2.38.9000/source) 翻看源码，这个网站对 glibc 整个源码生成了 Tags，可以实现对任意一个 symbol 在整个源码树下的 Definition/Reference/Implementation 跳转。 
...
感觉和这行代码有关：
![](images/d026ef66-698f-48f0-a67d-bd9a2d517777.png)
...
![](images/c0893acd-fc1e-4120-9232-eb91f47df758.png)

#### sym
sym 变量是 section .dynstr  的地址
![](images/a89dca9c-0e3c-43c4-b688-05488ddb1a91.png)
...
![](images/c7cd473e-3df5-48a6-9165-70d49f5dd279.png)
展开后就是下面一大串：
![](images/807080aa-864d-4fdf-bf6d-8846d195aaec.png)
...
![](images/457816e5-c3aa-4f5a-8041-275005ec82c4.png)
...
![](images/b9ec4313-aa63-4fda-ac8f-0e6e857d70ed.png)
...
![](images/d9eaae62-2337-44a1-8bcd-6698bad8f552.png)
...
```
数25 位试试看：
![](images/18d47349-92bb-429a-b8d3-3e92cd41b64b.png)
...
- **参数 `*l`:**
dl_fixup 的参数 *l 就是一个巨大的结构体变量，里面保存了 ELF 的各个段的偏移地址，这些地址会被频繁用到从而计算出需要的地址，如 rel_addr 就是被计算出来的。
```
pwndbg> ptype l
type = struct link_map {
    Elf64_Addr l_addr;
...
这里让 reloc_addr = value(printf 的值），还记得吗，reloc_addr
