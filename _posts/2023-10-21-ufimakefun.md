...
我的 UFI WIFI Stick 的原厂 Firmware 就已经开启了 adbd，使用  `adb reboot bootloader` 直接就进到原厂 fastboot 了：
...
> 请注意权限问题：如果Linux 的 udev 规则没有自动给当前用户添加 usb 设备读写权限，请使用 sudo 执行 adb 和 fastboot [color=red]

在第一次启动后OpenWRT会调整 rootfs 分区的结束扇区到 EMMC 的末尾，随后重启。
...
ssh 登陆 openwrt 后 `ip addr` 可以看到 eth0 设备。

将 UFI 作为一个小形服务器，DHCP Client 模式，挂在路由器后面，这就需要调整 br-lan，取消 eth0 的桥接模式：

![](/images/7a7f26eb-742a-4f2b-a71c-85b5b455959c.png)

在 `cgi-bin/luci/admin/network/network` 页面内的 `Interfaces` 子页面内，添加一个 Interface ，名为 WiredExtend，对应的物理设备为 eth0，Protocal 为 DHCP Client 模式：
...
OpenWRT 默认的 br-lan 地址为 192.168.1.1，这可能会与上级路由器冲突，需要修改 br-lan 为 其他地址如 192.168.4.1，加入 UFI WIFI 信号的设备 IP 地址将变为 192.168.4.1/24：
...
默认情况下，只有加入 OpenWRT WIFI 的设备能 ssh 上 OpenWRT，如果想从上级网络访问 OpenWRT，需要更改监听的 Interface
...
实际上 UFI003_MB_V02 支持USB模式自动切换，但需要修改内核 dts 树：
...
由于我构建的 ImmortalWrt 还未引入整个 patch ，所以还是得靠这个不优雅的startup脚本实现自动切换。
...
> BUGs ：操作 mmc0:: 会导致Kernel crash，可能会导致 emmc 损坏。[color=red]

我一般设置red led 旨在 kernel painic 的时候给我亮起，这时候我就知道 Kernel 崩溃了，其他时候led灯全部关闭。
...
Please Referto: https://en.wikipedia.org/wiki/Qualcomm_EDL_mode

- 按住 BPC 上的按钮不放，插入 USB 接口后 UFI 上电进入 EDL 模式
- adb reboot bootloader & fastboot oem reboot-edl 进入 EDL 模式


通过 Miko 内自带 EDL 端口驱动，可以很方便的安装：
![](/images/e4cd613e-f71f-4466-9577-3242d65977f2.png) [color=green]

## 清空 EMMC 分区表
从 EMMC 0 扇区开始进行破坏性写入一个 100m 大小的空白 blank.img 到 EMMC 中，旧的分区表就无了：
![](/images/9a9fd84d-4e70-4c06-9aa2-a4370fd59b47.png)

> 注意将 UFI 设备连接到 Vmware 虚拟机内而不是 Host 机，你可能需要反复重连确保 Miko 正确找到 EDL 设备。[color=red]



> 注意网上流传的 Miko Service Tool Pro 基本上是 “Cracked Software”，主程序由 `Loader.exe` 启动，安全性未知。[color=red]
> 像我一样有洁癖的人士建议在虚拟机内操作。
...
> 86226dce4f2782dfaa91bc0002317e4cb2cb7693  UFI003_MB_V02_EDL.7z [color=red]
...
理论上吧 aarch64 架构的 Qemu ELF 可执行文件直接放到 OpeenWRT 中跑就可以，不过我没尝试。有空的友友们可以试试看。

## 修改 rootfs 和 boot 镜像

> 你可以修改 rootfs 和 boot 镜像定制自己的 OpenWRT 系统 [color=red]，配合 UFI 上电过程中的 Serial Console Logs 定位具体的问题。
