...
```
Thanks to `lkiuyu/immortalwrt`, all UFI's Kernel/Packages/Firmeware integral to this branch so we can easily build our own Image for UFI stick `:)`
...
Before build Openwrt, Install required package and make sure u have stable network connectivity :
```bash
...
You need at least 50GB size of disk
...
Defiantly we need luci interface, so select luci packages.
I use nginx with ssl as server luci web interface:
![](/images/e40f2481-6664-4c6c-aff0-50d9f8e504e6.png)

UFI have modem so luci need modemmanager.
...
# Errors
go-bootstrap can not installed on arm64 system
